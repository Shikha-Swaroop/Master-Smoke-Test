...
public class TestNGReportListener implements IReporter {
	private static final Logger logger = Logger.getLogger(TestNGReportListener.class);
	...
	protected void writeBody() {
		...
		writeEnvironmentDetails();
		writer.println("</body>");
	}

	private void writeEnvironmentDetails() {
		writer.println("<br></br>");
		writer.println("<br></br>");
		writer.println("<h3>Host Machine - Environment Variables </h3>");
	    writer.println("<table>");
	    writer.print("<tr>");
	    writer.print("<th>Variable</th>");
	    writer.print("<th>Value</th>");
	    writer.println("</tr>");
	    for (String key : System.getenv().keySet()) 
	    	writer.println("<tr> <td>" + key + "</td> <td>"+ System.getenv().get(key) + "</tr>");

	    writer.println("</table>");
	    writer.println("<br></br>");
	    
	    writer.println("<h3>JAVA</h3>");
	    writer.println("<table>");
	    writer.print("<tr>");
	    writer.print("<th>JVM Setting</th>");
	    writer.print("<th>Value</th>");
	    writer.println("</tr>");
	    for (Object key : System.getProperties().keySet())
    		writer.println("<tr> <td>" + key.toString() + "</td> <td>"+ System.getProperties().get(key) + "</tr>");
	    writer.println("</table>");
	    writer.println("<br></br>");
	    
	    writer.println("<h3>Application Configuration Key-Value pairs</h3>");
	    writer.println("<table>");
	    writer.print("<tr>");
		...
		for (Field field : Config.class.getDeclaredFields())
			try {
				writer.println("<tr> <td>" + field.getName() + "</td> <td>" + field.get(field) + "</tr>");
			} catch (Exception e) {
	...
	protected void writeSuiteSummary() {
		...
		if (testIndex > 1) {
			...
			writeTableHeader(integerFormat.format(totalRetriedTests), (totalRetriedTests > 0 ? "num attn" : "num"));
	...
	protected void writeScenarioSummary() {
		...
		for (SuiteResult suiteResult : suiteResults) {
			...
			for (TestResult testResult : suiteResult.getTestResults()) {
				...
				scenarioIndex += writeScenarioSummary(testName + " &#8212; retried", testResult.getRetriedTestResults(),
						"retried", scenarioIndex);
	...
	protected static class TestResult {
		 ...
		 * order).
		 */
		protected static final Comparator<ITestResult> RESULT_COMPARATOR = Comparator
		...
		/** @return the results for failed SendfaxTests (possibly empty) */
		...
		/** @return the results for skipped SendfaxTests (possibly empty) */
		...
		/** @return the results for passed SendfaxTests (possibly empty) */
